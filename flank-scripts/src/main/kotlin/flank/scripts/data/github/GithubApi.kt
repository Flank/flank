...
// ============= HTTP GITHUB API =============
suspend fun getPrDetailsByCommit(
    commitSha: String,
    githubToken: String,
    repo: String = flankRepository
): Result<List<GithubPullRequest>, Exception> =
...
suspend fun getGitHubPullRequest(
    ...
    issueNumber: Int,
    repo: String = flankRepository
): Result<GithubPullRequest, Exception> =
...
suspend fun getGitHubIssueList(
    githubToken: String,
...
suspend fun getGitHubCommitList(
    githubToken: String,
    parameters: Parameters = emptyList(),
    repo: String = flankRepository
): Result<List<GitHubCommit>, Exception> =
...
suspend fun getGitHubWorkflowRunsSummary(
    ...
    parameters: Parameters = emptyList(),
    repo: String = flankRepository
): Result<GitHubWorkflowRunsSummary, Exception> =
    Fuel.get("$URL_BASE/$repo/actions/workflows/$workflow/runs", parameters)
        .appendGitHubHeaders(githubToken)
        .awaitResult(GithubWorkflowRunsSummaryDeserializer)
        .mapClientErrorToGithubException()

suspend fun postNewIssueComment(
    githubToken: String,
    issueNumber: Int,
    payload: GitHubCreateIssueCommentRequest,
    repo: String = flankRepository
): Result<GitHubCreateIssueCommentResponse, Exception> =
...
suspend fun postNewIssue(
    githubToken: String,
    payload: GitHubCreateIssueRequest,
    repo: String = flankRepository
): Result<GitHubCreateIssueResponse, Exception> =
...
suspend fun setLabelsToPullRequest(
    githubToken: String,
