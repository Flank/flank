...
suspend fun getGitHubIssueList(githubToken: String, parameters: Parameters = emptyList()) =
    Fuel.get("$URL_BASE/issues", parameters)
        .appendGitHubHeaders(githubToken)
        .awaitResult(GithubPullRequestListDeserializer)
        .mapErrorToGithubException()

suspend fun getGitHubCommitList(githubToken: String, parameters: Parameters = emptyList()) =
    Fuel.get("$URL_BASE/commits", parameters)
        .appendGitHubHeaders(githubToken)
        .awaitResult(GitHubCommitListDeserializer)
        .mapErrorToGithubException()

suspend fun getGitHubWorkflowRunsSummary(githubToken: String, workflow: String, parameters: Parameters = emptyList()) =
...
// POST
suspend fun postNewIssueComment(githubToken: String, issueNumber: Int, payload: GitHubCreateIssueCommentRequest) =
...
suspend fun postNewIssue(githubToken: String, payload: GitHubCreateIssueRequest) =
...
// PATCH
fun patchIssue(githubToken: String, issueNumber: Int, payload: GitHubUpdateIssueRequest) =
