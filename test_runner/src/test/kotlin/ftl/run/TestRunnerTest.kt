...
class TestRunnerTest {
    ...
    fun `Verify getDownloadPaths localResultDir false`() {
        ...
        val (downloadFile, fallbackFile) = TestRunner.getDownloadPaths(args, gcsPath)
        assertThat(downloadFile).isEqualTo(Paths.get(localResultDir, gcsPath))
        assertThat(fallbackFile).isEqualTo(Paths.get(localResultDir, parsed.objName, parsed.shardName, parsed.fileName))
    }
    ...
    fun `Verify getDownloadPaths localResultDir true`() {
        ...
        val (downloadFile, fallbackFile) = TestRunner.getDownloadPaths(args, gcsPath)
        assertThat(downloadFile).isEqualTo(Paths.get(localResultDir, parsed.afterObjName))
        assertThat(fallbackFile).isEqualTo(Paths.get(localResultDir, parsed.shardName, parsed.fileName))
    }
